...
function binary_image = DWT_thres_MSE(foreground_image_path, background_image_path)
    ...
    % 2. 使用中值濾波器去噪
    filtered_foreground = medfilt2(foreground, [3 3]);
    filtered_background = medfilt2(background, [3 3]);
    % 3. 使用二維小波轉換 (Haar DWT)
    [LL_fg, ~, ~, ~] = dwt2(filtered_foreground, 'haar');
    [LL_bg, ~, ~, ~] = dwt2(filtered_background, 'haar');
    %imshow(uint8(LL_fg));
    % 4. 背景減除
